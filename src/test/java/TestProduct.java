...
public class TestProduct {
	...
	public void testgetStock() {
		...
		Product checkProduct = checkList.get(0); // Get the particular product from the 0 index
		assertEquals(1, checkProduct.itemCode);// Check the product is correctly added in ArrayList
	}
	...
	public void testgetFilterStock() {
		...
		fliterCheckList = ProductManager.getFilterStockByCategory("laptop");// store the filtered list in checkList from productDetails ArrayList
		Product checkProduct = fliterCheckList.get(0);// get particular product from checkList store it in checkProduct
		assertEquals("Laptop", checkProduct.itemCategory);// check the given category is correctly added in checkProduct
	}
}
